...
def load_wic(wic_path='external/wic-tsv', folder='Development', setname='dev'):
    ...
    for example, definition in zip(open(examples_path, encoding="utf8"), open(definitions_path, encoding="utf8")):
...
def run_train(args):
    ...
    with open(results_path, 'w') as results_f:  # store results in WiC's format
        for wic_idx, wic_entry in enumerate(load_wic(args.wic_path)):
            ...
            pred = clf.predict([[s1_sim, s2_sim, s3_sim]])[0]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Evaluation of WiC-TSV solution using LMMS and LogReg for binary classification.')
    ...
    parser.add_argument('-clf_path', help='Path to .pkl LR classifier', required=True)
    ...
    results_path = 'data/results/wic.classify.%s_2.txt' % args.eval_set
